...
--Using Parameters, transaction queries
...
BEGIN 
	...
	--Get the Donor's ID
	SELECT	D.DonorID INTO DonorID
	FROM	Donor D
	WHERE	D.LastName = DonorLastName
	AND		D.FirstName = DonorFirstName;


	--Get the Campaign's ID
	SELECT		C.CampaignID INTO CampaignID
	FROM		Campaign C
	WHERE		C.CampaignDate = CampaignDate
	AND			C.CampaignLocation = CampaignLocation;
	...
	INSERT INTO Donation
        (DonationID, DonorID, CampaignID, DonationDate, DonationLocation, AmountDonated)
	VALUES	
        (CONCAT(DonorID, CampaignID), DonorID, CampaignID, SYSDATE, DonationLocation, AmountDonated);
...
--Query 1
--DESCRIPTION: Find average amount received by each receiver.
